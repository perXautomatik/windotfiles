...
$success_content = @{}
$a = [Linq.Enumerable]::ToDictionary($success_results, [Func[object,object]] { param($r) $r.RepoName }, [Func[object,object]] { param($r) Get-Content $r.GitFile })
$success_content.Keys = $a.Keys
$success_content.Values = $a.Values
...
foreach ($result in $results | Where-Object {$_.Success -match $f}) {
    ...
    if ($success_content.Key -ccontains $repoName) {
        ...
        $content = $success_content.Get($repoName)
