...
foreach ($file in $files) {
    ...
    $results += $res | % {
      ...
      [PSCustomObject]@{
        FileLocation = $file
        FunctionName = Find-GroupName -groupStart $_.start -Tokens $tokenz
...
$results = $results | ? { $_.FunctionName.type -eq "CommandArgument"} | select @{n="functionName"; e={$_.FunctionName.content}},@{n="FileLocation"; e={$_.FileLocation.resolvedTarget}},FunctionBody | Sort-Object FunctionName, FileLocation
