...
function not-insideOtherGroup
{
  ...
  if($subr.type -contains "groupStrart" )
  {
    return ($subr.type -contains "groupEnd" )
  }
  else
  {return $true}
  ...
  # Find the last group start token that has a smaller index than the group end token
  $groupStart = $Tokens | Where-Object {$_.Type -eq "CommandArgument" -and $_.Start -lt $groupStart.Start -and (not-insideOtherGroup -ob $_ -x $groupStart.Start )} | Select-Object -Last 1
