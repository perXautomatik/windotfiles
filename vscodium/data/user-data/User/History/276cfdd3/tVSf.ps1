...
function Get-GroupContent {
  ...
  $range = $Tokens | Where-Object {$_.Start -ge $GroupStart.Start -and $_.End -le $GroupEnd.End}

  # Join the content of each token in the range and return it
  return ($range | ForEach-Object {$_.Content}) -join ""
}
...
foreach ($file in $files) {
    ...
    $groups = $ends | select start, @{name = "end" ; expres={ (Find-GroupStart -groupend $_.start -Tokens $tokenz) }} 

    $res =  $groups | select start,end, @{ name = "functionBody" ; express={ Get-GroupContent -GroupStart $_.start -GroupEnd $_.end -Tokens $tokenz } }

    $result += $res | % {
      # Create a custom object with the file location, function name and function body
      [PSCustomObject]@{
        FileLocation = $file
        FunctionName = $tokenz[$_.start-1].content
