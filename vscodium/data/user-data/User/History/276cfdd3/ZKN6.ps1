...
foreach ($file in $files) {
    ...
    # Loop through each function token
    foreach ($functionToken in $functionTokens) {
      # Get the function name from the content of the token
      $functionName = $functionToken.Content

      # Find the matching end token for the function
      $endToken = $tokens | Where-Object { $_.Type -eq 'GroupEnd' -and $_.Start -gt $functionToken.Start } | Select-Object -First 1

      # Get the function body from the content between the function and end tokens
      $functionBody = $content.Substring($functionToken.Start, $endToken.End - $functionToken.Start)

      if ($type -eq “command” ) {

        $subRangeFromHere = $tokenz[$currentPos..$tokenz.Count]

        $nextCommandPos = $subRangeFromHere | ForEach-Object { $innerINdex++; $_ } | ForEach-Object { if( $_.type -eq “command”) { $innerIndex } } | Select-Object -First 1
        if ( $nextCommandPos -gt $currentPos ) {
          $tokenz[$currentPos..$nextCommandPos+1] | ForEach-Object { $onlyContent += $_.content }
