...
function Sync-FilesByRelativePath {
    ...
    param ($x,$y);
            New-Object -TypeName PSObject -Property @{
                x = $x;
                xName = Split-Path -Path $x -Leaf                            
                y = $y;
                yName = Split-Path -Path $y -Leaf                                                        
            }
        }
    )

    $OutputArray = [System.Linq.Enumerable]::ToArray($LinqJoinedData) ; $LinqJoinedData.Where({!($_.xName -eq $_.yName) -and ($_.x -eq $_.y)})
