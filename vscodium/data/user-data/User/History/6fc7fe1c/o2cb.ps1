...
function Sync-FilesByRelativePath {
    ...
    $OutputArray = $ToModifyFileList | Select-Object *, @{Name='Table2'; Expression={ $d = $_; $ReferenceFileList | ?{($_.name -eq $d.name ) -and ( $_.relative -ne $d.relative)}}}
    $q = ($OutputArray | ?{ $_.table2.count -gt 0 })
    
    foreach ($ToModifyFile in $q) 
    {
        
	    # Get the file name by splitting the relative path
        $referencefile = $ToModifyFile.table2 | select -First 1
            
        $StaticReference = (Join-Path -Path $Reference -ChildPath $ReferenceFile.relative)
        $ToModifyFile = (Join-Path -Path $ToModify -ChildPath $ReferenceFile.table2.relative)
