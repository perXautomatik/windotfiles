...
function Sync-FilesByRelativePath {
    ...
    $LinqJoinedData = [System.Linq.Enumerable]::Join(
        $ReferenceFileList,
        $ToModifyFileList,
        [System.Func[Object,string]]{param ($r);$r},
        [System.Func[Object,string]]{param ($t);$t},
        [System.Func[Object,Object,Object]]{
    param ($r,$t);
            New-Object -TypeName PSObject -Property @{
                reference = $r;
                referenceName = Split-Path -Path $r -Leaf                            
                ToModify = $t;
                ToModifyName = Split-Path -Path $t -Leaf
                name = ($t -eq $r)
                folder = ((Split-Path -Path $t -Leaf) -eq (Split-Path -Path $r -Leaf) )                                                        
            }
        }
    )
    
    $LinqJoinedData.Where({($_.referenceName -eq $_.ToModifyName) -and !($_.reference -eq $_.ToModify)})
    ...
    $q = ($OutputArray | Group-Object -Property ToModifyName,ToModify )
