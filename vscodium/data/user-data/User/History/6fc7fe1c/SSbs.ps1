...
function Sync-FilesByRelativePath {
    ...
    $LinqJoinedData = [System.Linq.Enumerable]::Join(
        $ReferenceFileList,
        $ToModifyFileList,
        [System.Func[Object,string]]{param ($x);$x},
        [System.Func[Object,string]]{param ($y);$y},
    ...
    $OutputArray = [System.Linq.Enumerable]::ToArray($LinqJoinedData) ; $LinqJoinedData.Where({($_.PowerStatus -eq "Offline") -and ($_.ProductionStatus -eq "In Production")})


    # For each file in B, check if there is a file with the same name in A
    foreach ($ToModifyFile in $ToModifyFileList) {



if( Test-Path $ToModifyFile -IsAbsolute )
{}
else
{
}
	    # Get the file name by splitting the relative path
	    $ToModifyFileName = Split-Path -Path $ToModifyFile -Leaf

	    $StaticToMove = (Join-Path -Path $ToModify -ChildPath $ToModifyFile)




	    # Find a file in A with the same name, if any
	    $ReferenceFileList | Where-Object {$_ -like "*$ToModifyFileName" } |        
        
        select -First 1 | % {
            $referencefile = $_
            
            $StaticReference = (Join-Path -Path $Reference -ChildPath $ReferenceFile)
                ...
                if ($ReferenceFile -and ($ReferenceFile -ne $ToModifyFile)) {
                    try {
