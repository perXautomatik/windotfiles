...
function Split-Branches-ByFiles
{
    ...
    foreach ($rPath in $relPath) {
        ...
        $nameArray = @(
            (($ExecutionContext).SessionState.Path.GetUnresolvedProviderPathFromPSPath($rPath)).basename,
            $bName,
            (($ExecutionContext).SessionState.Path.GetUnresolvedProviderPathFromPSPath($rPath)).Parent)
