...
function createMultipleBranches {
    ...
    foreach ($pair in $dictionary.GetEnumerator()) {
        ...
        if (git branch --no-track $name) {
            ...
            if (git update-ref refs/heads/$name $ref)
			{
...
function Main
{
    ...
    foreach ($rPath in $relPath) {
        ...
        $nameArray = @((get-item -path $rPath).basename,$bName,(get-item -path $rPath).Parent)
...
$Pattern = "^(.{13})(.{11})(\S{3,})\s(.*)$"; 
$Data = Get-Content 'B:\Unsorted\tempRepo\.git\filter-repo\analysis\path-all-sizes.txt' -Encoding UTF8 | select -Skip 1 ; 
     ...
     [PSCustomObject]@{ $Headers[0] = $Match[1].Value.Trim() ;
         $Headers[1] = $Match[2].Value.Trim(); $Headers[2] = $Match[3].Value.Trim();
          $Headers[3] = $Match[4].Value.Trim() ;
           filename=( "B:\Unsorted\tempRepo\" + $match[4].Value.Trim() -replace("/","\") | split-path -Leaf ) }}  | ?{  $table -contains $_.filename } | %{ [PSCustomObject]@{ path=$_._path_name; alias=$_.filename } }