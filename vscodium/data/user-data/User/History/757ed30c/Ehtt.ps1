...
function createValidBranchName {
    ...
    .PARAMETER array
    An array of strings that will be used to create the branch name. This parameter can accept values from pipeline.
    ...
    .EXAMPLE
    @("foo","bar","baz") | createValidBranchName -stringEmbedding "{0}_{1}-{2}" -repoPath "C:\Users\user\repo" -checkAgainstRepo
    Creates a branch name "bar_foo-baz" and checks if it already exists in the repository at "C:\Users\user\repo".
    ...
    # Run once before processing pipeline input
    begin {
        if ($checkAgainstRepo) {
            ...
            # Store the output of git branch --list in a variable
            $branches = git branch --list
        }
    }

    # Run for each pipeline input
    process {
        # Create a branch name from the array and string embedding parameters
        $returnName = $stringEmbedding -f $_

        # Remove any invalid characters from the branch name (spaces, dots, slashes, etc.)
        $regex = '[\s\.\/\\:*?""<>|~^]'
        ...
        if ($checkAgainstRepo) {
            # Check if the returnName already exists among branch names in the repository using the stored variable
            if ($branches | Select-String -Pattern $returnName) {
...
function Split-Branches-ByFiles
{
    ...
    # Loop through each relative path in the filtered array
$relPath | CreateValidBranchName -stringEmbedding "{0}_{1}-{2}" -checkAgainstRepo | ForEach-Object {
    # Add a hashtable of name, ref and relative path to the creation array
    $creationArray += @{"name"=$_;"ref"=$ref;"path"=$rPath}
}
    ...
    createMultipleBranches -dictionary ($zx) | ForEach-Object {
        # Add a hashtable of branch name and relative path to the successful hashtable
        $successful[$_.name] = $yx[$_.name]
