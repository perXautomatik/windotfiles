...
function ResolveConflict {
    ...
    # Switch on the flag value to use different strategies for resolving the objects
    switch ($Flag) {
        # If the flag is Tree, use git read-tree and git write-tree to merge the tree objects
        Tree {
        ...
        # If the flag is Message, use git fmt-merge-msg to format the message strings into a single message string
        Message {
            # Create an empty string to store the input for git fmt-merge-msg
            $input = ""

            # Loop through each message string in the input array
            foreach ($message in $Objects) {
        ...
        # If the flag is Metadata, use a custom logic to merge the metadata hashtable into a single hashtable
        Metadata {
            # Create an empty hashtable to store the merged metadata
            $mergedMetadata = @{}

            # Loop through each metadata hashtable in the input array
            foreach ($metadata in $Objects) {
