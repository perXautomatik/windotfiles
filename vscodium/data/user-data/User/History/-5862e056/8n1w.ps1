...
function Insertion-Sort {
    ...
    $unsorted = $list
    # Initialize an empty array for the sorted list
    $sortedx = @()
    ...
    foreach ($current in $unsorted) {
        ...
        # Insert the current element in the correct position by slicing and concatenating arrays
        $sortedx = @($sortedx[0..($index-1)]) + @($current) + @($sortedx[$index..($sortedx.Count-1)])
    }
