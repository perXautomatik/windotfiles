...
function Insertion-Sort {
    ...
    for ($i = 0; $i -lt $unsorted.Count; $i++) {
        ...
        for($j = 0; $j -lt $sortedx.Length; $j++ )
        {
            $verdict = checkPredicate $current $sortedx[$j]

            if(($j -gt 0) -and ($j -lt $sortedx.Length) -and $verdict) # middle of array
            {
            ...
            elseif (($j -eq $sortedx.Length) -and ($sortedx.Length -gt 0)) { # end of array 
