...
function Add-Gist {
    ...
    # Assign a SQL statement to the CommandText property
    $commandText = "INSERT INTO gists (id, filename, visibility, description, filecontent) VALUES ('some_id', 'some_filename', 'public', 'some_description', NULL);"
...
$SqlConnection.ConnectionString = "Server=localhost;Database=test;User Id=sa;Password=1234;"
...
$gObjs | %{
    $item =  $_ ;
    $gistExists = Test-GistExists -Id $item.id -Filename $item.filename -Command $item.Command
    ...
    if (-not $gistExists) {
        if (Add-Gist -Id $item.id -Filename $item.filename -Visibility $visibility -Description $description -Command $item.Command) {
            Write-Output "Inserted gist with ID: $item.id and filename: $item.filename for user: $Username without content"
        }
        else {
            Write-Verbose "Failed to insert gist with ID: $item.id and filename: $item.filename for user: $Username without content: $_"
        }
    }
