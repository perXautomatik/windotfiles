...
function Test-GistExists {
    ...
    $Command.CommandText = "SELECT id, filename FROM gists WHERE id='$Id' AND filename='$Filename'"
...
$gObjs | %{ 
    ...
    if (-not $gistExists) {
        if (Add-Gist -Id $id -Filename $filename -Visibility $visibility -Description $description -Command $Command) {
            ...
            Write-Error "Failed to insert gist with ID: $id and filename: $filename for user: $Username without content: $_"
            continue
        }
    }
