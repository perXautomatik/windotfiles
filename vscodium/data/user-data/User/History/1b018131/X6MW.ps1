    ...
    try {
        ...
        if ($errorMessage -match "UNIQUE constraint failed") {
            ...
            if ($errorMessage -match "UNIQUE constraint failed: gists.(\w+) \((.+)\)") {
                ...
                # Write an error message that indicates a unique key violation but does not specify which column or value caused it
                Write-Error "Failed to insert gist into the table because there is already another gist with some of the same values. Please check your input and try again."
            }
        }
        else {
