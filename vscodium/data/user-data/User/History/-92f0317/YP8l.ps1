...
function Get-RelativeFileList {
	...
	[string]$Path
        ...
        $relativeFiles = $files | ForEach-Object {
            GetRelative $path $_.FullName
    }
    # Check if the resulting list is the same as before replacement
    if (Compare-Object -ReferenceObject ($files.FullName) -DifferenceObject $relativeFiles) {
        # Return the relative file list
        return $relativeFiles
    }
    else {
        # Throw an error
        throw "The resulting list is the same as before replacement"
    }
}
    ...
    #Validate-Parameter -Name SourceFile -Value $SourceFile -ScriptBlock {Test-Path -Path $_ -PathType Leaf} -ErrorMessage "The source file $_ does not exist or is not a file"
   # Validate-Parameter -Name DestinationFile -Value $DestinationFile -ScriptBlock {Test-Path -Path $_ -PathType Container} -ErrorMessage "The destination file $_ does not exist or is not a folder"

    # Use git mv to move the file and commit with the message
    try {
        # Use -f to force overwrite existing files
        $U =  "git mv -f '"+$SourceFile.replace("'","''")+"' '"+$DestinationFile.replace("'","''")+"'"
        $U = invoke-expression $U -ErrorAction Stop
        $A = "git commit -m "+"""+$CommitMessage+"""
        $A = invoke-expression $A  -ErrorAction Stop
        Write-Output "Successfully moved and committed $SourceFile to $DestinationFile"
    }
    catch {
        # Catch any errors and display them
        $er = $_.ScriptStackTrace;
        Write-Error "Failed to move and commit $U to $A"        
        Write-Error $er
    }
}
...
function Move-FileToMatchRelativePath {
	...
	try {
            Move-FileWithGit -SourceFile $SourceFile -DestinationFile $destinationPath -CommitMessage "$sourceRelativepath -> $relativeTargetPath"
        }
	catch {
	...
	try {
            Move-Item -path $SourceFile -Destination $destinationPath -Force -Verbose
        }
	catch {
...
function Sync-FilesByRelativePath {
    ...
    foreach ($ToModifyFile in $q) 
    {
        ...
        $StaticToMove = (Join-Path -Path $ToModify -ChildPath $ToModifyFile.relative)
        $StaticReference = (Join-Path -Path $reference -ChildPath $ReferenceFile.relative)
