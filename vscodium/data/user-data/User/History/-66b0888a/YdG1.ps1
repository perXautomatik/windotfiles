...
Import-Module .\Split-TextByRegex.ps1
...
"@        
    ...
    It 'Splits the text by digits and returns the matches' {
        ...
        $output[0].StartIndex | Should -Be 50 # Check that the first match starts at index 50
        $output[0].EndIndex | Should -Be 59 # Check that the first match ends at index 59
    ...
    It 'Splits the text by five-letter words and returns the matches' {
        ...
        # Use the Should keyword to verify the expected behavior
        $output | Should -Not -BeNullOrEmpty # Check that the output is not null or empty
        $output | Should -HaveCount 6 # Check that the output has six elements
        $output[0].StartIndex | Should -Be 26 # Check that the first match starts at index 26
        $output[0].EndIndex | Should -Be 30 # Check that the first match ends at index 30
        $output[0].Value | Should -Be "abcde" # Check that the first match value is "abcde"
        $output[5].StartIndex | Should -Be 57 # Check that the last match starts at index 57
        $output[5].EndIndex | Should -Be 61 # Check that the last match ends at index 61
        $output[5].Value | Should -Be "uvwxy" # Check that the last match value is "uvwxy"
    }
    ...
    It 'Splits the text by whitespace and returns the matches' {
        ...
        # Use the Should keyword to verify the expected behavior
        $output | Should -Not -BeNullOrEmpty # Check that the output is not null or empty
        $output | Should -HaveCount 11 # Check that the output has eleven elements
