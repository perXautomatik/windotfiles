...
# Define a function to convert a key to a value from a hashtable
function KeyToValue {
    <#
    .SYNOPSIS
    Converts a key to a value from a hashtable.
    .PARAMETER Key
    The key to look up in the hashtable.
    .PARAMETER Values
    The hashtable that contains the key-value pairs.
    .EXAMPLE
    KeyToValue -Key "Red" -Values @{Red = 1; Green = 2; Blue = 3}
    #>
    ...
    if (!$Values.ContainsKey($Key)) {
        # Throw an exception if the key is not recognized
...
# Define a function to ensure that a registry key is deleted
function EnsureRegistryKeyDeleted {
    <#
    .SYNOPSIS
    Ensures that a registry key is deleted.
    .PARAMETER Path
    The path of the registry key to delete.
    .EXAMPLE
    EnsureRegistryKeyDeleted -Path "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Run\SomeApp"
    #>
    ...
    if (Test-Path -LiteralPath "Registry::$Path") {
        # Delete the registry key and log the update
        DoUpdate "Registry key $Path deleted" {
            Remove-Item -LiteralPath "Registry::$Path" -Force -Recurse | Out-Null
        }
    } else {
        # Log that the registry key is already missing
        LogIdempotent "Registry key $Path is already missing"
    }
}

# Define a function to get a registry value from a given path and name
function GetRegistryValue {
     <#
     .SYNOPSIS
     Gets a registry value from a given path and name.
     .PARAMETER Path
     The path of the registry key that contains the value.
     .PARAMETER Name
     The name of the registry value to get.
     .EXAMPLE
     GetRegistryValue -Path "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" -Name "SomeApp"
     #>
     # Validate the parameters
     [CmdletBinding()]
     param (
     ...
     # Try to get the registry value and return it, or return null if an error occurs
     try {
...
# Define a function to ensure that a registry value has a given type and value, or is deleted if the value is null
function EnsureRegistryValue {
     <#
     .SYNOPSIS
     Ensures that a registry value has a given type and value, or is deleted if the value is null.
     .PARAMETER Path
     The path of the registry key that contains the value.
     .PARAMETER Name
     The name of the registry value to set or delete.
     .PARAMETER Type
     The type of the registry value to set. Valid types are String, Binary, DWord, QWord, MultiString, ExpandString, None, Unknown.
     .PARAMETER Value
     The value of the registry value to set. If null, the registry value will be deleted. If a script block, it will be invoked with the current value as an argument and return the new value or null.
     .EXAMPLE
     EnsureRegistryValue -Path "HKEY_LOCAL_MACHINE\SOFTWARE\Microsoft\Windows\CurrentVersion\Run" -Name "SomeApp" -Type String -Value "C:\Program Files\SomeApp\SomeApp.exe"
     #>
     # Validate the parameters
     [CmdletBinding()]
     param (
         ...
         [Parameter(Mandatory=$true)]
         [ValidateNotNullOrEmpty()]
         [ValidateSet("String", "Binary", "DWord", "QWord", "MultiString", "ExpandString", "None", "Unknown")]
         [string]$Type,
     ...
     # Check if the registry key exists
     if (!(Test-Path -LiteralPath "Registry::$Path")) {
         # If the value is null, log that the registry item is already missing and return
         if (!$hasValue) {
             LogIdempotent "Registry item $Path\$Name is already missing"
             return
         }

         # Otherwise, create the registry key and log the update
         DoUpdate "Registry path $Path created" {
             $newItem = New-Item -Path "Registry::$Path" -Force -ErrorAction:SilentlyContinue
             # If the registry key cannot be created, log a warning and return
             if (!$newItem) {
     ...
     # If the registry key object is not null, try to open it with write access or read-only access
     if ($key) {
         try {
         ...
         # Get the current value of the registry item
         $currentValue = $item.GetValue($Name)

         # If the value is a script block, invoke it with the current value as an argument and get the new value or null
         if ($hasValue) {
         ...
         # If the registry key object is null and not in dry run mode, log a warning and return
         if (IsDryRun) {
             $canWrite = $true
         } else {
     ...
     # If the type is binary, convert the value to a byte array and compare it with the current value using Compare-Object
     if ("Binary" -eq $Type) {
         $Value = [byte[]]$Value
         $areDifferent = @(Compare-Object $Value $currentValue -SyncWindow 0).Length -ne 0
     } else {
         # Otherwise, compare the value with the current value using equality operator
         $areDifferent = ($currentValue -ne $Value)
     }

     # If the values are different, update or delete the registry item accordingly and log the update
     if ($areDifferent) {
         if ($hasValue) {
             ...
             # If write access is available, set the registry item value and type
             if ($canWrite) {
                 DoUpdate $msg {
                     $item.SetValue($Name, $Value, (KeyToValue -Key "$Type" -Values ([Microsoft.Win32.RegistryValueKind]::GetNames([Microsoft.Win32.RegistryValueKind]) | ForEach-Object {$_, $_})))
                 }
             } else {
                 # Otherwise, log a warning that the registry item cannot be updated
                 LogWarn "Registry item $Path\$Name cannot be updated"
             }
         } else {
             # If write access is available, delete the registry item value
             if ($canWrite) {
                 DoUpdate "Registry item $Path\$Name removed, previous value was '$currentValue'" {
                     $item.DeleteValue($Name)
                 }
             } else {
                 # Otherwise, log a warning that the registry item cannot be deleted
                 LogWarn "Registry item $Path\$Name cannot be deleted"
             }
         }
     } else {
         # If the values are the same, log that the registry item is already set or missing as expected
         if ($hasValue) {
             LogIdempotent "Registry item $Path\$Name value is already set to '$Value'"
         } else {
             LogIdempotent "Registry item $Path\$Name
