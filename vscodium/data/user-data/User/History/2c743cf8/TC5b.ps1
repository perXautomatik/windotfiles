...
function Create-JsonFiles {
  ...
  foreach ($Subfolder in $Subfolders) {
      ...
      New-Item -ItemType File -Path $jsX -ErrorAction SilentlyContinue
      ...
      $regex = '[\[\]",\s\t]*'
      $JsonString = ConvertTo-Json -InputObject $Entries | % {$_ -replace $regex,"" }
      ...
      New-Item -ItemType File -Path $jsonFilex -ErrorAction SilentlyContinue
