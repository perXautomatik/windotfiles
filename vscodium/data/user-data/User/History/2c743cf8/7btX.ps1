...
function Create-JsonFiles {
  ...
  foreach ($Subfolder in $Subfolders) {
      ...
      $JsonFile = Join-Path -Path $Subfolder.FullName -ChildPath ($Subfolder.Name + ".json")
      ...
      foreach ($InnerSubfolder in $InnerSubfolders) {
          ...
          $Entry = @{
              Name = $InnerSubfolder.Name
              Path = $InnerSubfolder.FullName
          }
          # Add the entry to the array
          $Entries += $Entry
      }
      # Convert the array to a JSON string and write it to the JSON file
      $JsonString = ConvertTo-Json -InputObject $Entries
      Set-Content -Path $JsonFile -Value $JsonString

      # Create a new .js file inside the subfolder with JSON containing the given text
      $JsFile = Join-Path -Path $Subfolder.FullName -ChildPath ($Subfolder.Name + ".js")
      New-Item -ItemType File -Path $JsFile
      ...
      Set-Content -Path $JsFile -Value $JsString
  }
}


# Call the function with the example path as an argument
Create-JsonFiles -FolderPath $ExamplePath