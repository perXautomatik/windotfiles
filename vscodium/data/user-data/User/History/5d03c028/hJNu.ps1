...
$repo = "B:\PF\PowerShellProjectFolder\TodoProjects\gitListSubmodules and git files in tree.ps1" #git config --get remote.origin.url
...
foreach ($file in $files) {
    ...
    $temp = New-TemporaryFile | Rename-Item -NewName { $_ -replace 'tmp$', 'git' } -PassThru | % { rm $_ - ; $_ | New-Object -TypeName Directory}  -
