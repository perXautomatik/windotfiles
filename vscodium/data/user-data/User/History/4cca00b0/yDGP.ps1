...
function Extract-Functions {
	...
	try {
		$t = Tokenize-Script -Ps1File $Ps1File -ErrorAction Stop
	}
	catch {
	...
	if ($t.FunctionTokens.Count -eq 0) {
		Write-Error "No functions detected in $Ps1File"
		return
	}
	else {
		$d = $t.FunctionTokens.count
		$y = $t.LinesToRemove.count
		Write-Host "$d _ $y"
		Write-Verbose $t.FunctionTokens
		Foreach ( $a in $t.FunctionTokens ) {                
	...
	try {
		Write-Files -Ps1File $Ps1File -FunctionTokens $t.FunctionTokens -LinesToRemove $t.LinesToRemove -ErrorAction Stop
	}
	catch {
...
function Write-Files {
	...
	try {
		...
		Write-Error "Failed to verify psm1 file: $_"
		return
	}

	# Call the Remove-Lines function to remove the lines from the script content
	try {
		$scriptContent = Remove-Lines -Ps1File $Ps1File -LinesToRemove $LinesToRemove -ErrorAction Stop
	}
	catch {
	...
	# Call the Write-Functions function to write the extracted functions to the psm1 file
	try {
		Write-Functions -Psm1File $Psm1File -FunctionTokens $FunctionTokens -ErrorAction Stop
	}
	catch {
		Write-Error "Failed to Write-Functions: $_"
		return
	}

	# Call the Set-Content function to remove the extracted functions from the ps1 file
	try {
		Set-Content -Path $Ps1File -Value $scriptContent -ErrorAction Stop
	}
	catch {
