...
$argx = "C:\Users\CrRoot\AppData\Local\Temp\smartgit480401022183064819\tool_11608756714323891237_selectionFile"
 ; invoke-command  -scriptblock { param($file) if (Test-Path $file -PathType Leaf) { $lines = Get-Content $file; $a = $lines[0] ; $b = $lines[1] ;  BCompare "$a" "$b"  } } -ArgumentList $argx  -InDisconnectedSession
