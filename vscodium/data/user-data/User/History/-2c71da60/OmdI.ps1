...
$argx = "C:\Users\CrRoot\AppData\Local\Temp\smartgit480401022183064819\tool_11608756714323891237_selectionFile"
 ; invoke-command  -scriptblock { param($file) if (Test-Path $file -PathType Leaf) { $lines = Get-Content $file ;  $bc = scoop which beyondcompare ; & $bc $lines[0] $lines[1] } else { Write-Host "The file does not have two lines." } } else { Write-Host "The file does not exist." } -ArgumentList $argx
