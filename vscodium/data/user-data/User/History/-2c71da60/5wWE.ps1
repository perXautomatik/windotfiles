...
invoke-command -InDisconnectedSession -scriptblock { param($file) if (Test-Path $file -PathType Leaf) { $lines = Get-Content $file; $a = $lines[0] ; $b = $lines[1] ;  BCompare "$a" "$b"  } } -ArgumentList @{file = "$argx"} 
