...
$p1 = "'${filePath}'" ; 
$p1 = convert-path $p1  ; 
 $p2 ="'${repositoryRootPath}'" ;
   $p2 = convert-path $p2;
    invoke-command -scriptblock {param($param1,$param2) ;   (coreutils realpath --relative-to="$param2" ( $param1 | split-path -parent )).replace('\','/') >> (Join-Path -Path $param2 -ChildPath '.gitignore')} -ArgumentList $p1, $p2

$argx = "C:\Users\CrRoot\AppData\Local\Temp\smartgit480401022183064819\tool_11608756714323891237_selectionFile"
 ; Invoke-Expression  -scriptblock { param($file) if (Test-Path $file -PathType Leaf) { $lines = Get-Content $file ;  $bc = scoop which beyondcompare ; & $bc $lines[0] $lines[1] } else { Write-Host "The file does not have two lines." } } else { Write-Host "The file does not exist." } -ArgumentList $argx
