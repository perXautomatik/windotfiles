...
function comparatorX { param([string]$q,[string]$qz) return ($args[0].StartsWith($args[1])) }
...
function Sort-List {
    ...
    for ($i = 1; $i -lt $list.Count; $i++) {
        ...
        while ($j -ge 0 -and $comparator.Invoke($current, $list[$j])) {
...
$sorted = Sort-List -list $folders -comparator $comparator
